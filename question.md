...
<details>
    ...
    <h2>WeakMap</h2>
    <ol>
        <li>weakmap只接受对象作为键名</li>
        <li>weakmap的键名所引用的对象是弱引用</li>
        <li>不可遍历</li>
    </ol>
    ...
    <h2>js执行会阻塞DOM树的解析和渲染，那么css加载会阻塞DOM树的解析和渲染吗？</h2>
    <ol>
        <li>css加载不会阻塞DOM树的解析，会阻塞DOM树的渲染</li>
        <li>原因：加载完后可能会导致多次重绘</li>
        <li>css加载会阻塞后面的js语句的执行</li>
    </ol>
    <h2>重绘和重排</h2>
    <ol>
        <li>不管页面发生了重绘还是重排，都会影响性能，最可怕的是重排，会使我们付出高额的性能代价，所以我们应尽量避免</li>    
    </ol>
    <h2>跨域</h2>
    <ol>
        <li>
